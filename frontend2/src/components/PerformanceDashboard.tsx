...
const PerformanceDashboard: React.FC = () => {
  ...
  const fetchMetrics = async () => {
    ...
    try {
      ...
      const mockHealth: SystemHealth = {
        status: mockMetrics.cpuUsage > 80 ? 'CRITICAL' : mockMetrics.cpuUsage > 60 ? 'WARNING' : 'HEALTHY',
  ...
  const slowQueryColumns: ColumnsType<SlowQuery> = [
    ...
    },
    {
      ...
      key: 'executionTime',
      render: (time: number) => (
        <Tag color={time > 2000 ? 'red' : time > 1000 ? 'orange' : 'green'}>
          {time}ms
        </Tag>
      ),
    },
              ...
              <Col span={6}>
                <Card size="small">
                  <Statistic
                    ...
                    suffix="ms"
                    valueStyle={{ color: metrics?.responseTime && metrics.responseTime > 1000 ? '#f5222d' : '#52c41a' }}
                  />
                </Card>
              </Col>
              <Col span={6}>
              ...
              <Col span={6}>
                <Card size="small">
                  <Statistic
                    ...
                    suffix="%"
                    valueStyle={{ color: metrics?.cacheHitRate && metrics.cacheHitRate > 80 ? '#52c41a' : '#faad14' }}
                  />
                </Card>
              </Col>
          ...
          <div>
            <Row gutter={[16, 16]}>
              ...
              <Col span={12}>
                ...
                <Text strong style={{ display: 'block', marginTop: '8px' }}>执行频率：</Text>
                <div style={{ marginTop: '4px' }}>
                  {selectedQuery.frequency}/min
                </div>
              </Col>
            </Row>
          </div>
