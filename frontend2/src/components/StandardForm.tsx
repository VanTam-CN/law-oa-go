...
export interface FormField {
  name: string;
  label: string;
  type: 'text' | 'password' | 'textarea' | 'select' | 'multiple' | 'date' | 'time' | 'datetime' | 'daterange' | 'timerange' | 'checkbox' | 'radio' | 'switch' | 'number' | 'rate' | 'slider' | 'cascader' | 'tree' | 'upload' | 'autocomplete';
  required?: boolean;
  disabled?: boolean;
  readonly?: boolean;
  hidden?: boolean;
      ...
      fields.forEach(field => {
        if (field.type === 'date' || field.type === 'time' || field.type === 'datetime') {
          if (processedValues[field.name] && dayjs.isDayjs(processedValues[field.name])) {
            processedValues[field.name] = processedValues[field.name].format(field.format || 'YYYY-MM-DD');
          }
