...
export const financeStatsUtils = {
   ...
   * @returns 发票统计数据
   */
  calculateInvoiceStats: (invoices: Array<{
    amount: number;
    dueDate: string;
    paidDate?: string;
    status: 'pending' | 'paid' | 'overdue';
   ...
   * @returns 费用统计数据
   */
  calculateExpenseStats: (expenses: Array<{
    amount: number;
    status: 'pending' | 'approved' | 'rejected';
  }>) => {
    const stats = {
   ...
   * @returns 净收入和利润率
   */
  calculateNetIncome: (invoiceStats: { total: number; paid: number }, expenseStats: { approved: number }) => {
