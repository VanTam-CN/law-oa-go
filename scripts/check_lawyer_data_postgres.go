...
func main() {
	...
	rows2, err := db.Query(`
		...
		WHERE c.lawyer_id = $1 AND (
			cl.company ILIKE '%阿里%' OR cl.company ILIKE '%淘宝%' OR cl.company ILIKE '%天猫%' OR
			cl.name ILIKE '%阿里%' OR
			c.opposing_party ILIKE '%阿里%' OR c.opposing_party ILIKE '%淘宝%' OR c.opposing_party ILIKE '%天猫%' OR
			c.case_name ILIKE '%阿里%' OR c.case_name ILIKE '%淘宝%' OR c.case_name ILIKE '%天猫%'
		)
	`, lawyerID)
	if err != nil {
	...
	db.QueryRow("SELECT COUNT(*) FROM lawyers").Scan(&lawyerCount)
	...
	if techCaseCount == 0 {
		...
		fmt.Println("🔧 创建测试数据的步骤:")
		fmt.Println("1. 创建阿里巴巴客户")
		fmt.Println("2. 为张伟律师创建阿里巴巴相关的案件")
		fmt.Println("3. 然后测试为字节跳动创建新案件的冲突检测")
	}
}