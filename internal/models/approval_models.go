...
type ApprovalRequest struct {
	...
	// 冲突审批冻结证据。详情接口必须返回这些权威字段，前端才能校验
	// 元数据和审批快照是否仍指向同一条检测记录。
	ConflictCheckID   string     `json:"conflict_check_id,omitempty" gorm:"column:conflict_check_id;type:varchar(100);index"`
	ConflictRiskLevel string     `json:"conflict_risk_level,omitempty" gorm:"column:conflict_risk_level;type:varchar(30)"`
	ConflictCheckTime *time.Time `json:"conflict_check_time,omitempty" gorm:"column:conflict_check_time"`
	ConflictResult    string     `json:"conflict_result,omitempty" gorm:"column:conflict_result;type:json"`
...
func (a *ApprovalWorkflow) BeforeCreate(tx *gorm.DB) error {
	if a.ID == "" {
		a.ID = uuid.NewString()
	}
	return nil
...
func (a *ApprovalTemplateV2) BeforeCreate(tx *gorm.DB) error {
	if a.ID == "" {
		a.ID = uuid.NewString()
	}
	return nil
...
func (a *ApprovalNotification) BeforeCreate(tx *gorm.DB) error {
	if a.ID == "" {
		a.ID = uuid.NewString()
	}
	return nil
