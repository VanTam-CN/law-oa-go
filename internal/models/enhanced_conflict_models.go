...
type WaiverApplication struct {
	...
	// 申请类型
	WaiverType     string `gorm:"type:varchar(40);comment:豁免类型" json:"waiver_type"`
	WaiverCategory string `gorm:"type:varchar(40);comment:豁免类别" json:"waiver_category"`
	...
	// 状态管理
	Status         string     `gorm:"type:varchar(30);default:'DRAFT';comment:状态" json:"status"`
	SubmissionDate *time.Time `gorm:"comment:提交日期" json:"submission_date,omitempty"`
	ReviewPriority string     `gorm:"type:varchar(20);default:'MEDIUM';comment:审核优先级" json:"review_priority"`

	// 审批流程
	CurrentStage     *string    `gorm:"type:varchar(40);comment:当前阶段" json:"current_stage,omitempty"`
...
// Waiver applications are compliance evidence. Their workflow state may
// change, but the application itself must never be physically removed.
func (WaiverApplication) BeforeDelete(*gorm.DB) error {
	return errors.New("waiver applications are append-only; revoke or expire instead")
}
...
type WaiverApprovalRecord struct {
	...
	ApproverRole  string  `gorm:"type:varchar(40);comment:审批人角色" json:"approver_role"`

	// 审批决定
	Decision         string  `gorm:"type:varchar(30);comment:审批决定" json:"decision"`
...
type WaiverSignature struct {
	...
	// 签名方信息
	SignerType         string  `gorm:"type:varchar(20);comment:签名方类型" json:"signer_type"`
	...
	// 签名技术信息
	SignatureMethod      string    `gorm:"type:varchar(30);comment:签名方式" json:"signature_method"`
	...
	// 验证信息
	VerificationStatus string     `gorm:"type:varchar(20);default:'PENDING';comment:验证状态" json:"verification_status"`
...
type WaiverMonitoringRecord struct {
	...
	// 监控基本信息
	MonitoringType        string     `gorm:"type:varchar(40);comment:监控类型" json:"monitoring_type"`
	...
	// 风险评估
	CurrentRiskLevel     *string `gorm:"type:varchar(20);comment:当前风险等级" json:"current_risk_level,omitempty"`
	RiskTrend            *string `gorm:"type:varchar(20);comment:风险趋势" json:"risk_trend,omitempty"`
	...
	// 合规状况
	ComplianceStatus  string `gorm:"type:varchar(30);default:'UNDER_REVIEW';comment:合规状况" json:"compliance_status"`
	...
	// 状态
	Status string `gorm:"type:varchar(20);default:'SCHEDULED';comment:状态" json:"status"`
...
func (ProfessionalConflictCheckRequest) BeforeDelete(*gorm.DB) error {
	return errors.New("conflict check requests are append-only; cancel instead")
}
