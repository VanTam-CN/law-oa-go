...
type InboxItem struct {
	...
	// 关联信息
	UserID     uint   `json:"user_id" gorm:"not null;index,priority:1;index,priority:1"`
	...
	// 优先级与时间
	Priority    string     `json:"priority" gorm:"size:20;not null;default:'medium';index,priority:2;comment:优先级: critical/high/medium/low"`
	DueDate     *time.Time `json:"due_date" gorm:"index,priority:3"`
	DueDateType string     `json:"due_date_type" gorm:"size:50;comment:日期类型: hearing/appeal/evidence等"`

	// 状态
	IsRead      bool       `json:"is_read" gorm:"default:false;index,priority:2"`
	ReadAt      *time.Time `json:"read_at"`
	IsCompleted bool       `json:"is_completed" gorm:"default:false;index,priority:3"`
...
type LawyerConflictPool struct {
	...
	LawyerID uint `json:"lawyer_id" gorm:"not null;index,priority:1;comment:律师ID"`
	...
	EntityNameStandard string `json:"entity_name_standard" gorm:"size:200;not null;index,priority:2;index,priority:1;comment:工商标准名称"`
	EntityTaxID        string `json:"entity_tax_id" gorm:"size:50;index,priority:2"`
	...
	CaseID           uint   `json:"case_id" gorm:"not null;index;comment:关联案件ID"`
...
type ConflictReport struct {
	...
	// 检测信息
	CheckedBy       uint      `json:"checked_by" gorm:"not null;index;comment:检测人ID"`
	...
	// 检测对象
	ClientName    string `json:"client_name" gorm:"size:200;not null;index;comment:客户标准名称"`
	...
	// 检测结果
	RiskLevel        string `json:"risk_level" gorm:"size:20;not null;index;comment:风险等级: CRITICAL/HIGH/MEDIUM/LOW/PASS"`
...
type CompanyAPICall struct {
	...
	// API信息
	APIProvider   string `json:"api_provider" gorm:"size:50;not null;index,priority:2;comment:API提供商"`
	...
	// 搜索信息
	SearchKeyword       string `json:"search_keyword" gorm:"size:200;not null;index;comment:搜索关键词"`
	MatchedCompanyName  string `json:"matched_company_name" gorm:"size:200;comment:匹配到的公司名称"`
	MatchedCompanyTaxID string `json:"matched_company_tax_id" gorm:"size:50;index;comment:匹配到的税号"`
...
type ConflictScanJob struct {
	...
	// 扫描配置
	ScanType  string `json:"scan_type" gorm:"size:20;not null;index,priority:1;comment:扫描类型: daily/weekly/manual"`
	ScanScope string `json:"scan_scope" gorm:"size:50;default:'all';comment:扫描范围: all/new_cases/lawyer"`

	// 扫描状态
	Status string `json:"status" gorm:"size:20;default:'pending';index;comment:状态: pending/running/completed/failed"`
...
type DocumentLock struct {
	...
	LockedBy    uint      `json:"locked_by" gorm:"not null;index;comment:锁定人ID"`
...
type DocumentVersionNew struct {
	...
	DocumentID uint `json:"document_id" gorm:"not null;uniqueIndex:uk_doc_version,priority:1;index,priority:1;comment:文档ID"`
	...
	// 状态
	IsCurrent bool `json:"is_current" gorm:"default:false;index,priority:2;comment:是否当前版本"`
}
...
type DocumentIndexQueue struct {
	...
	DocumentID uint   `json:"document_id" gorm:"not null;index;comment:文档ID"`
	FilePath   string `json:"file_path" gorm:"size:500;not null"`
	MimeType   string `json:"mime_type" gorm:"size:100"`

	// 索引状态
	Status       string     `json:"status" gorm:"size:20;default:'pending';index;comment:状态: pending/processing/completed/failed"`
...
type CaseFolderTemplate struct {
	...
	// 分类
	CaseType  string `json:"case_type" gorm:"size:50;index;comment:适用案件类型"`
...
type Contract struct {
	...
	// 关联信息
	CaseID   *uint `json:"case_id" gorm:"index;comment:关联案件ID"`
	ClientID uint  `json:"client_id" gorm:"not null;index;comment:客户ID"`
	...
	// 合同状态
	Status string `json:"status" gorm:"size:20;default:'draft';index;comment:状态: draft/active/suspended/completed/cancelled"`

	// 补充协议
	ParentContractID *uint  `json:"parent_contract_id" gorm:"index;comment:主合同ID(补充协议时使用)"`
	...
	PaymentMilestones []PaymentMilestone `json:"payment_milestones,omitempty" gorm:"foreignKey:ContractID"`
}
...
type PaymentMilestone struct {
	...
	ContractID uint       `json:"contract_id" gorm:"not null;index;comment:合同ID"`
	...
	Status     string  `json:"status" gorm:"size:20;default:'pending';index;comment:状态: pending/billed/paid/overdue"`
	InvoiceID  *uint   `json:"invoice_id" gorm:"index;comment:关联发票ID"`
...
type Invoice struct {
	...
	// 关联信息
	ContractID  *uint `json:"contract_id" gorm:"index;comment:合同ID"`
	...
	OriginalInvoiceID *uint   `json:"original_invoice_id" gorm:"index;comment:原发票ID(红冲时)"`
	...
	// 开票流程
	Status              string     `json:"status" gorm:"size:20;default:'draft';index;comment:状态: draft/submitted/approved/issued/received/cancelled"`
	...
	Client    *Client           `json:"client,omitempty" gorm:"foreignKey:ClientID"`
	Contract  *Contract         `json:"contract,omitempty" gorm:"foreignKey:ContractID"`
	Milestone *PaymentMilestone `json:"milestone,omitempty" gorm:"foreignKey:MilestoneID"`
}
...
type Payment struct {
	...
	InvoiceID   uint      `json:"invoice_id" gorm:"not null;index;comment:发票ID"`
	Amount      float64   `json:"amount" gorm:"type:decimal(15,2);not null;comment:回款金额"`
	PaymentDate time.Time `json:"payment_date" gorm:"not null;index;comment:付款日期"`
	...
	Status      string     `json:"status" gorm:"size:20;default:'confirmed';index;comment:状态: pending/confirmed/rejected"`
...
type BadDebtRecord struct {
	...
	// 审批流程
	Status        string     `json:"status" gorm:"size:20;default:'pending';index;comment:状态: pending/approved/rejected"`
	ApprovedBy    *uint      `json:"approved_by" gorm:"comment:审批人ID"`
...
type CommissionRecord struct {
	...
	// 关联信息
	ContractID uint  `json:"contract_id" gorm:"not null;index;comment:合同ID"`
	PaymentID  uint  `json:"payment_id" gorm:"not null;index;comment:回款ID"`
	CaseID     *uint `json:"case_id" gorm:"comment:案件ID"`

	// 受益人信息
	BeneficiaryID   uint   `json:"beneficiary_id" gorm:"not null;index,priority:1;comment:受益人ID"`
	BeneficiaryRole string `json:"beneficiary_role" gorm:"size:50;not null;index,priority:2;comment:角色: source/lawyer/assistant"`
	...
	Status       string     `json:"status" gorm:"size:20;default:'pending';index,priority:3;comment:状态: pending/calculated/paid/cancelled"`
...
type NotificationQueue struct {
	...
	// 触发信息
	TriggerType string `json:"trigger_type" gorm:"size:50;not null;index,priority:1;comment:触发类型"`
	TriggerID   uint   `json:"trigger_id" gorm:"not null;index,priority:2;comment:触发记录ID"`
	CaseID      *uint  `json:"case_id" gorm:"comment:关联案件ID"`

	// 接收人信息
	RecipientType    string `json:"recipient_type" gorm:"size:20;not null;index,priority:1;comment:接收人类型: client/lawyer/admin"`
	RecipientID      uint   `json:"recipient_id" gorm:"not null;index,priority:2;comment:接收人ID"`
	...
	// 状态
	Status   string `json:"status" gorm:"size:20;default:'pending';index;comment:状态: pending/approved/sent/cancelled/failed"`
...
type NotificationTemplate struct {
	...
	// 适用信息
	Channel       string `json:"channel" gorm:"size:20;not null;index,priority:1;comment:适用渠道: email/sms/wechat"`
	RecipientType string `json:"recipient_type" gorm:"size:20;not null;comment:接收人类型: client/lawyer/admin"`
	TriggerEvent  string `json:"trigger_event" gorm:"size:100;not null;index,priority:2;comment:触发事件"`
...
type SensitiveWord struct {
	...
	WordType    string `json:"word_type" gorm:"size:50;not null;index;comment:词类型: political/porn/violence/other"`
	Category    string `json:"category" gorm:"size:50;index;comment:分类"`
...
type ContentFilterLog struct {
	...
	// 来源信息
	ContentType     string `json:"content_type" gorm:"size:50;not null;index;comment:内容类型: notification/email/document"`
	ContentID       uint   `json:"content_id" gorm:"not null;index;comment:内容ID"`
...
type WechatInvitation struct {
	...
	// 状态
	Status string `json:"status" gorm:"size:20;default:'pending';index;comment:状态: pending/accepted/expired/cancelled"`
...
type CaseEthicalWallWhitelist struct {
	...
	CaseID    uint      `json:"case_id" gorm:"not null;uniqueIndex:uk_case_user,priority:1;index;comment:案件ID"`
	UserID    uint      `json:"user_id" gorm:"not null;uniqueIndex:uk_case_user,priority:2;index;comment:用户ID"`
...
type EthicalWallAccessLog struct {
	ID           uint      `json:"id" gorm:"primarykey"`
	CaseID       uint      `json:"case_id" gorm:"not null;index;comment:案件ID"`
	...
	AccessResult string    `json:"access_result" gorm:"size:20;not null;index;comment:访问结果: allowed/denied"`
...
type ClientTrustAccount struct {
	...
	// 状态
	Status   string     `json:"status" gorm:"size:20;default:'active';index;comment:状态: active/frozen/closed"`
...
type ClientTrustTransaction struct {
	...
	AccountID       uint   `json:"account_id" gorm:"not null;index;comment:账户ID"`
	TransactionCode string `json:"transaction_code" gorm:"size:50;uniqueIndex;not null;comment:交易编号"`

	// 交易信息
	TransactionType string  `json:"transaction_type" gorm:"size:20;not null;index;comment:交易类型: deposit/deposit_refund/withdraw/transfer"`
	Amount          float64 `json:"amount" gorm:"type:decimal(15,2);not null;comment:金额"`
	...
	// 状态
	Status       string     `json:"status" gorm:"size:20;default:'pending';index;comment:状态: pending/completed/cancelled"`
	CompletedAt  *time.Time `json:"completed_at" gorm:"comment:完成时间"`
...
type OffboardingRecord struct {
	...
	UserID      uint      `json:"user_id" gorm:"not null;index;comment:离职用户ID"`
	...
	// 状态
	Status string `json:"status" gorm:"size:20;default:'pending';index;comment:状态: pending/in_progress/completed/cancelled"`
...
type OffboardingTransferDetail struct {
	ID            uint   `json:"id" gorm:"primarykey"`
	OffboardingID uint   `json:"offboarding_id" gorm:"not null;index;comment:交接记录ID"`
	...
	// 状态
	TransferStatus string     `json:"transfer_status" gorm:"size:20;default:'pending';index;comment:状态: pending/completed/failed"`
...
type TokenRevocationLog struct {
	ID             uint   `json:"id" gorm:"primarykey"`
	UserID         uint   `json:"user_id" gorm:"not null;index;comment:用户ID"`
	RevocationType string `json:"revocation_type" gorm:"size:50;not null;index;comment:撤销类型: offboarding/password_reset/manual"`
...
type DataImportTask struct {
	...
	// 导入信息
	ImportType string `json:"import_type" gorm:"size:50;not null;index;comment:导入类型: client/case/contract/document"`
...
type DataImportError struct {
	...
	ImportTaskID uint   `json:"import_task_id" gorm:"not null;index;comment:导入任务ID"`
...
type UserExtension struct {
	SupervisorID      *uint   `json:"supervisor_id" gorm:"column:supervisor_id;index;comment:上级/合伙人ID"`
...
type CaseExtension struct {
	EthicalWallEnabled     bool       `json:"ethical_wall_enabled" gorm:"column:ethical_wall_enabled;default:false;index;comment:是否启用隔离墙"`
