...
type IntegrationExecutor struct {
	*BaseExecutor
	...
	currentPhase   int
	phaseResults   map[string]*TestResult
}
...
func NewIntegrationExecutor(options *TestExecutorOptions, logger TestLogger, metrics TestMetrics) *IntegrationExecutor {
...
func (e *IntegrationExecutor) GetExecutorType() TestType {
	return TestTypeIntegration
}

// Setup 设置集成测试执行器
func (e *IntegrationExecutor) Setup(ctx context.Context, executionCtx *ExecutionContext) error {
	// 调用基础设置
...
// Teardown 清理集成测试执行器
func (e *IntegrationExecutor) Teardown(ctx context.Context, executionCtx *ExecutionContext) error {
...
func (e *IntegrationExecutor) executeMainTest(ctx context.Context, test *TestCase, result *TestResult, executionCtx *ExecutionContext) error {
	...
	for i, phase := range e.testPhases {
		...
		// 如果阶段失败且不是可选的，停止执行
		if phaseResult.Status == TestStatusFailed && !e.isOptionalPhase(phase) {
...
func (e *IntegrationExecutor) parseTestPhases(test *TestCase) error {
	// 从测试步骤中解析阶段配置
				...
				for _, phaseData := range phases {
					if phaseMap, ok := phaseData.(map[string]interface{}); ok {
						phase := TestPhase{
							...
							Type:         TestType(getString(phaseMap, "type", "api")),
...
func (e *IntegrationExecutor) checkPhaseDependencies(phase TestPhase) error {
	for _, dependency := range phase.Dependencies {
		...
		if e.phaseResults[dependency].Status == TestStatusFailed {
...
func (e *IntegrationExecutor) executePhaseSetup(ctx context.Context, phase TestPhase, executionCtx *ExecutionContext) error {
...
func (e *IntegrationExecutor) executePhaseTests(ctx context.Context, phase TestPhase, executionCtx *ExecutionContext) (*TestResult, error) {
	// 创建阶段结果
	phaseResult := &TestResult{
	...
	if err := executor.Setup(ctx, executionCtx); err != nil {
		phaseResult.Status = TestStatusFailed
		phaseResult.Error = &TestError{
			Type:    "setup_error",
			Message: err.Error(),
		}
	...
	// 设置阶段结果为成功
	phaseResult.Status = TestStatusPassed
...
func (e *IntegrationExecutor) executePhaseTeardown(ctx context.Context, phase TestPhase, executionCtx *ExecutionContext) error {
...
func (e *IntegrationExecutor) executePhaseStep(ctx context.Context, step TestStep, executionCtx *ExecutionContext, testType TestType) error {
	switch testType {
	case TestTypeAPI:
...
func (e *IntegrationExecutor) mergeBatchResults(phaseResult *TestResult, testResults []*TestResult) {
...
func (e *IntegrationExecutor) mergeTestResult(phaseResult *TestResult, testResult *TestResult) {
	// 合并断言
...
func (e *IntegrationExecutor) mergePhaseResults(result *TestResult) {
	...
	for _, phaseResult := range e.phaseResults {
		if phaseResult.Status == TestStatusPassed {
			successCount++
		} else {
			failureCount++
		}
...
func (e *IntegrationExecutor) recordIntegrationMetrics(result *TestResult) {
	// 记录总体执行时间
	...
	// 记录阶段分布
	phaseDistribution := make(map[TestType]int)
...
func (e *IntegrationExecutor) GetPhaseResults() map[string]*TestResult {
...
func (e *IntegrationExecutor) ClearTestPhases() {
	e.testPhases = make([]TestPhase, 0)
	e.phaseResults = make(map[string]*TestResult)
}
