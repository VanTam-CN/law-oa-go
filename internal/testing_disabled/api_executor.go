...
func NewAPIExecutor(options *TestExecutorOptions, logger TestLogger, metrics TestMetrics) *APIExecutor {
...
func (e *APIExecutor) GetExecutorType() TestType {
	return TestTypeAPI
}

// executeMainTest 执行API测试的主逻辑
func (e *APIExecutor) executeMainTest(ctx context.Context, test *TestCase, result *TestResult, executionCtx *ExecutionContext) error {
...
func (e *APIExecutor) buildRequest(test *TestCase, executionCtx *ExecutionContext) (*http.Request, error) {
	// 解析URL
	url := test.URL
	if url == "" {
	...
	if test.Body != nil {
		...
		case string:
			bodyData := e.substituteVariables(v, executionCtx.Variables)
...
func (e *APIExecutor) validateResponse(test *TestCase, resp *http.Response, body []byte, result *TestResult) error {
	// 验证状态码
	...
	// 验证JSON响应体
	if test.Expected != nil && test.Expected.Body != nil {
		var responseBody interface{}
		if err := json.Unmarshal(body, &responseBody); err == nil {
			// 深度比较JSON
			assertion := &TestAssertion{
				Name:     "body_json_deep_equal",
				Type:     "deep_equal",
			}
			...
			result.Assertions = append(result.Assertions, assertion)
			if !assertion.Passed {
				return fmt.Errorf("body deep equal assertion failed: %s", assertion.ErrorMessage)
			}
		} else {
			// 如果不是JSON，直接比较
			bodyStr := string(body)
...
func (e *APIExecutor) deepEqualJSON(expected, actual interface{}, assertion *TestAssertion) error {
...
func (e *APIExecutor) ExecuteNavigate(ctx context.Context, step TestStep, executionCtx *ExecutionContext) error {
	return fmt.Errorf("navigate step not applicable for API tests")
}

// ExecuteClick 执行点击步骤（API测试中不适用）
func (e *APIExecutor) ExecuteClick(ctx context.Context, step TestStep, executionCtx *ExecutionContext) error {
	return fmt.Errorf("click step not applicable for API tests")
}

// ExecuteFill 执行填充步骤（API测试中不适用）
func (e *APIExecutor) ExecuteFill(ctx context.Context, step TestStep, executionCtx *ExecutionContext) error {
	return fmt.Errorf("fill step not applicable for API tests")
}

// ExecuteWait 执行等待步骤
func (e *APIExecutor) ExecuteWait(ctx context.Context, step TestStep, executionCtx *ExecutionContext) error {
	// 检查等待条件
	if step.Target != "" {
		// 这里可以实现轮询等待，暂时使用简单等待
		return nil
...
func (e *APIExecutor) ExecuteScreenshot(ctx context.Context, step TestStep, result *TestResult) error {
	return fmt.Errorf("screenshot step not applicable for API tests")
}

// ExecuteJavaScript 执行JavaScript步骤（API测试中不适用）
func (e *APIExecutor) ExecuteJavaScript(ctx context.Context, step TestStep, result *TestResult, executionCtx *ExecutionContext) error {
	return fmt.Errorf("javascript step not applicable for API tests")
}

// ExecuteStep 执行单个步骤
func (e *APIExecutor) ExecuteStep(ctx context.Context, step TestStep, executionCtx *ExecutionContext) error {
	switch step.Type {
	case "navigate":
	...
	case "javascript":
		return fmt.Errorf("javascript step not applicable for API tests")
	default:
		e.logger.Warn("Unknown step type for API executor", "type", step.Type)
		return nil
	}
}