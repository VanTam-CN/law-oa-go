 ...
 * @returns 集成结果
 */
export const createApprovalWithConflict = async (request: IntegrationRequest): Promise<IntegrationResult> => {
  try {
 ...
 * @returns 集成结果
 */
export const createIntegratedApproval = async (request: IntegrationRequest): Promise<IntegrationResult> => {
  try {
 ...
 * @returns 冲突检测结果
 */
export const triggerConflictCheck = async (config: ConflictCheckConfig): Promise<ConflictCheckResult> => {
  try {
...
export const createCaseFromApproval = async (
  ...
  try {
    const response = await post(`/integration/approvals/${approvalId}/case`, { case_data: caseData });
    return response.data;
  } catch (error) {
 ...
 * @returns 验证结果
 */
export const validateIntegrationConfig = (config: IntegrationRequest): { isValid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  if (config.conflict_check_config) {
    ...
    if (!config.conflict_config.client_ids || config.conflict_check_config.client_ids.length === 0) {
