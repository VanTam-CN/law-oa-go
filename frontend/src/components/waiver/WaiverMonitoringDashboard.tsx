...
const WaiverMonitoringDashboard: React.FC<WaiverMonitoringDashboardProps> = ({
  ...
  const calculateCompletionRate = (task: WaiverMonitoringTask) => {
    if (task.completionHistory.length === 0) return 0;
      ...
      <Row gutter={16} style={{ marginBottom: 24 }}>
          ...
          <Card>
            <Statistic
              ...
              value={monitoringTasks.reduce((total, task) => {
                ...
                return total + task.completionHistory.filter(
                  completion => dayjs(completion.completedAt).format('YYYY-MM') === thisMonth &&
        ...
        <Col span={6}>
          <Card>
            <Statistic
              title="监控覆盖率"
              value={approvedWaivers.length > 0 ?
                Math.round((monitoringTasks.length / approvedWaivers.length) * 100) : 0}
              suffix="%"
              ...
              {monitoringTasks
                ...
                .map((completion, index) => (
                  <Timeline.Item
                    key={index}
                    color={
                      completion.status === 'COMPLETED' ? 'green' :
                      completion.status === 'PARTIALLY_COMPLETED' ? 'orange' : 'red'
                    }
                  >
                    <div>
                  ...
                  <div key={key} style={{ marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <Text>
                        {key === 'pending' ? '待审批' :
                         key === 'approved' ? '已批准' :
                         key === 'rejected' ? '已拒绝' : '已上报'}
                      </Text>
                      <Text strong>{value}</Text>
                    </div>
                    <Progress
                      percent={statistics.totalApplications > 0 ? (value / statistics.totalApplications) * 100 : 0}
                      showInfo={false}
                      size="small"
                    />
                  </div>
          ...
          <Descriptions column={2} bordered>
            ...
            <Descriptions.Item label="任务完成率">
              {monitoringTasks.length > 0 ?
                Math.round((monitoringTasks.reduce((total, task) =>
                  total + calculateCompletionRate(task), 0) / monitoringTasks.length)) : 0}%
            </Descriptions.Item>
          </Descriptions>
        </Card>
      </div>
        ...
        {selectedTask && (
          <Form form={completionForm} layout="vertical" onFinish={handleCompleteTask}>
            <div style={{ marginBottom: 16, padding: 16, background: '#f5f5f5', borderRadius: 6 }}>
              ...
              <br />
              <Text type="secondary">截止日期：{dayjs(selectedTask.nextDueDate).format('YYYY-MM-DD')}</Text>
            </div>
            ...
            <Form.Item label="证据材料">
              <Upload
                ...
                beforeUpload={() => false}
                onChange={({ fileList }) => {
                  setEvidenceFiles(fileList.map(file => file.originFileObj).filter(Boolean) as File[]);
                }}
              >
                ...
                >
                  <div>
                    ...
                    <Tag color="blue" style={{ marginLeft: 8 }}>
                      {completion.status === 'COMPLETED' ? '完全完成' :
                       completion.status === 'PARTIALLY_COMPLETED' ? '部分完成' : '未完成'}
                    </Tag>
                  </div>
                  ...
                  {completion.evidenceAttachments.length > 0 && (
                    <div style={{ marginTop: 8 }}>
                      <Text type="secondary">证据材料：{completion.evidenceAttachments.length} 个</Text>
                    </div>
                  )}
                </Timeline.Item>
              ))}
