...
const CreateCaseWizard: React.FC<CreateCaseWizardProps> = ({ visible, onCancel, onSuccess }) => {
        ...
        conflicts: (result.conflictCases || []).map((c) => ({
          ...
          impact:
            (c.riskLevel || 'low') === 'HIGH'
              ? '高影响'
              : (c.riskLevel || 'low') === 'MEDIUM'
                ? '中等影响'
                : '轻微影响',
          probability:
            (c.riskLevel || 'low') === 'HIGH' ? 85 : (c.riskLevel || 'low') === 'MEDIUM' ? 65 : 30,
          severity:
            (c.riskLevel || 'low') === 'HIGH' ? 4 : (c.riskLevel || 'low') === 'MEDIUM' ? 3 : 2,
          evidence: [
            {
              type: '系统检查记录',
  ...
  const handleSubmit = async () => {
    try {
      ...
      const caseData: CaseInfo = {
        ...
        status: '0',
        conflictCheckStatus: conflictResult?.status === 'passed' ? 'PASSED' : 'WARNING',
      ...
      >
        ...
        <Row gutter={[16, 16]}>
          <Col span={12}>
            ...
            >
              <Select placeholder='请选择案件类型' size='large' suffixIcon={<TeamOutlined />}>
                {caseTypes?.map((type) => (
          ...
          <Col span={12}>
            <Form.Item label='项目类型' name='projectType'>
              <Select placeholder='请选择项目类型' size='large' suffixIcon={<RocketOutlined />}>
                ...
                {clients?.map((client) => (
                  <Option key={client.clientId} value={client.clientId}>
                    ...
                    >
                      ...
                      <Tag color={client.clientType === '个人' ? 'blue' : 'orange'}>
                        {client.clientType}
                      </Tag>
                    </div>
                  </Option>
  ...
  const renderTeamAssignment = () => (
    <div style={{ background: '#fafafa', padding: '16px', borderRadius: '8px' }}>
      ...
      >
        <Row gutter={[16, 16]}>
          <Col span={12}>
            <Form.Item
              label={
                <Space>
                  <span style={{ fontWeight: 500 }}>主办律师</span>
                  <Tooltip title='负责案件整体把控和主要工作'>
                    <InfoCircleOutlined style={{ color: '#1E5A8D', fontSize: '14px' }} />
                  </Tooltip>
                </Space>
              }
              name='lawyerId'
            ...
            >
              ...
              >
                {lawyers?.map((lawyer) => {
                  const initials = lawyer.lawyerName?.slice(-2) || '律师'
                  return (
                    <Option
                      key={lawyer.lawyerId}
                      value={lawyer.lawyerId}
                      label={lawyer.lawyerName}
                      searchText={`${lawyer.lawyerName} ${lawyer.department} ${lawyer.specialty}`}
                    >
                      <div
                        style={{
                          display: 'flex',
          ...
          <Col span={12}>
            <Form.Item
              label={
                <Space>
                  <span style={{ fontWeight: 500 }}>协办律师</span>
                  <Text type='secondary' style={{ fontSize: '12px' }}>
                    (可选)
                  </Text>
                </Space>
        ...
        <Row gutter={[16, 16]}>
          <Col span={8}>
            <Form.Item name='isMajorRisk' valuePropName='checked' style={{ marginBottom: 0 }}>
              <Checkbox>
          ...
          <Col span={8}>
            <Form.Item name='isMassCase' valuePropName='checked' style={{ marginBottom: 0 }}>
              <Checkbox>
                <Space direction='vertical' size='small' style={{ textAlign: 'center' }}>
          ...
          <Col span={8}>
            <Form.Item name='isSensitiveCase' valuePropName='checked' style={{ marginBottom: 0 }}>
              <Checkbox>
                <Space direction='vertical' size='small' style={{ textAlign: 'center' }}>
                  <EyeOutlined style={{ fontSize: '20px', color: '#722ed1' }} />
      ...
      {conflictChecking ? (
        ...
        >
          <Space direction='vertical' size='large' style={{ width: '100%' }}>
            <Spin
              size='large'
              indicator={<LoadingOutlined style={{ fontSize: 48, color: '#fa541c' }} spin />}
            />
            <div>
        ...
        <ConflictCheckResult
                ...
                conflict.type === 'client'
                  ...
                  : conflict.type === 'opponent'
                    ...
                    : conflict.type === 'lawyer'
                      ...
                      : '案件冲突',
              riskLevel:
                conflict.level === 'high' ? 'HIGH' : conflict.level === 'medium' ? 'MEDIUM' : 'LOW',
          ...
          recommendations={
            conflictResult.recommendations?.map((r) => r.action || r.description) || []
          }
      ...
      <Space direction='vertical' style={{ width: '100%' }} size='large'>
        ...
        >
          <Descriptions column={2} bordered size='small'>
            ...
            <Descriptions.Item label='案件类型'>
              <Tag color='blue'>
                {caseTypes.find((t) => t.code === formData.caseType)?.name || formData.caseType}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item label='项目类型'>
              <Tag color='green'>
                {projectTypes.find((t) => t.code === formData.projectType)?.name ||
                  formData.projectType}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item label='开始日期'>
              {formData.startDate ? dayjs(formData.startDate).format('YYYY-MM-DD') : '未设置'}
            </Descriptions.Item>
            <Descriptions.Item label='预计结束日期'>
              {formData.endDate ? dayjs(formData.endDate).format('YYYY-MM-DD') : '未设置'}
            </Descriptions.Item>
          ...
          <Descriptions column={2} bordered size='small'>
            <Descriptions.Item label='委托人'>
              <Space>
                <Avatar size='small' icon={<UserOutlined />} />
                <Text strong>
                  {clients.find((c) => c.clientId === formData.clientId)?.clientName || '未选择'}
                </Text>
              </Space>
            </Descriptions.Item>
        ...
        >
          <Descriptions column={2} bordered size='small'>
            <Descriptions.Item label='主办律师'>
              <Space>
                <Avatar size='small' icon={<UserOutlined />} />
                <Text strong>
                  {lawyers.find((l) => l.lawyerId === formData.lawyerId)?.lawyerName || '未选择'}
                </Text>
                <Tag color='purple'>
                  {lawyers.find((l) => l.lawyerId === formData.lawyerId)?.position}
                </Tag>
              </Space>
            </Descriptions.Item>
            <Descriptions.Item label='协办律师'>
              {formData.assistingLawyerId ? (
                <Space>
                  <Avatar size='small' icon={<UserOutlined />} />
                  <Text>{lawyers.find((l) => l.id === formData.assistingLawyerId)?.name}</Text>
                  <Tag color='cyan'>
                    {lawyers.find((l) => l.id === formData.assistingLawyerId)?.position}
                  </Tag>
                </Space>
              ) : (
            ...
            <Descriptions.Item label='收费方式'>
              <Tag color='orange'>
                {billingMethods.find((m) => m.code === formData.billingMethod)?.name ||
          ...
          <Space wrap>
            ...
            {!formData.isMajorRisk && !formData.isMassCase && !formData.isSensitiveCase && (
        ...
        >
          {conflictResult ? (
            <Space direction='vertical' style={{ width: '100%' }} size='middle'>
              <Row gutter={24} align='middle'>
                ...
                <Col span={16}>
                  <Space direction='vertical' size='small' style={{ width: '100%' }}>
                    ...
                    <div>
                      <Text type='secondary' style={{ fontSize: '12px' }}>
                        发现冲突: {conflictResult.conflicts?.length || 0} 项 | 检查时间:{' '}
                        {conflictResult.checkTime || '刚刚'}
                      </Text>
                    </div>
                  </Space>
                </Col>
              ...
              {conflictResult.status !== 'passed' && (
                <Alert
                  message={conflictResult.status === 'warning' ? '注意事项' : '严重警告'}
                  description={
                    conflictResult.status === 'warning'
                      ? '该案件存在潜在利益冲突，请确认已充分了解相关风险并做好披露工作。建议在充分披露后谨慎接受委托。'
                      : '该案件存在严重利益冲突，强烈建议拒绝委托或采取回避措施。'
                  }
                  type={conflictResult.status === 'warning' ? 'warning' : 'error'}
                  showIcon
                />
              )}
            </Space>
        ...
        <Modal
          title={
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <RocketOutlined style={{ color: '#1E5A8D' }} />
              <div style={{ margin: 0 }}>
                <Title level={4} style={{ margin: 0, color: '#1E5A8D' }}>
                  新建案件
                </Title>
              </div>
            </div>
          ...
          footer={null}
          destroyOnClose
          wrapClassName='create-case-wizard-modal'
          styles={{
            body: {
              padding: 0,
              height: '75vh',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            >
              <div style={{ textAlign: 'center' }}>
                <Space size='large'>
                  {currentStep > 0 && (
                    <Button onClick={handlePrev} style={{ minWidth: '100px' }}>
                      上一步
                    </Button>
                  )}
                  ...
                  <Button onClick={onCancel} style={{ minWidth: '100px' }}>
                    取消
                  </Button>
                </Space>
              </div>
