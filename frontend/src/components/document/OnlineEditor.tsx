...
export const OnlineEditor: React.FC<OnlineEditorProps> = ({
  ...
  const [connectionStatus, setConnectionStatus] = useState<'connected' | 'disconnected' | 'saving'>('connected')
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (
        ...
        !event.origin.includes(process.env.REACT_APP_ONLYOFFICE_URL || '')
      ) {
        return
      }
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {lockStatus?.is_locked && (
              <Chip
                ...
                label={lockStatus.can_edit ? '已锁定（您）' : `已锁定（${lockStatus.locked_by_name}）`}
                size="small"
                color={lockStatus.can_edit ? 'success' : 'warning'}
              />
            )}
            {connectionStatus === 'saving' && (
              <Chip label="保存中..." size="small" color="info" />
            )}
      ...
      <DialogContent sx={{ p: 0, height: 'calc(100% - 72px)', display: 'flex' }}>
        <Box sx={{ width: '100%', height: '100%', position: 'relative' }}>
          ...
          {config && !loading && (
            <iframe
              ref={iframeRef}
              srcDoc={generateEditorHTML(config)}
              style={{
                width: '100%',
                height: '100%',
                border: 'none',
 ...
 */
function generateEditorHTML(config: OnlyOfficeConfig): string {
  const onlyOfficeURL = process.env.REACT_APP_ONLYOFFICE_URL || 'http://localhost:9090'
...
<body>
  ...
  <script>
    const docEditor = new DocsAPI.DocEditor("editor", {
      ...
      events: {
        onReady: function() {
          window.parent.postMessage('onDocumentReady', '*');
        },
        onDocumentStateChange: function() {
          window.parent.postMessage('onDocumentStateChange', '*');
        },
        onSave: function(event) {
          window.parent.postMessage('onSave', '*');
        },
