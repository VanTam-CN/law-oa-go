...
const getConflictTypeIcon = (type: string) => {
  switch (type) {
    ...
    case '商业竞争冲突': return <BankOutlined style={{ color: '#17a2b8' }} />;
    default: return <ExclamationCircleOutlined style={{ color: '#6c757d' }} />;
  }
};
      ...
      footer={[
        ...
        onNavigateCase && (
          <Button
            key="view"
            type="primary"
            onClick={() => onNavigateCase(conflictCase.caseId)}
          >
            查看案件详情
          </Button>
        )
      ...
      <div style={{ padding: '20px 0' }}>
            ...
            <Col span={8}>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginTop: '4px' }}>
                  {getConflictTypeIcon(conflictCase.conflictType || '')}
                  <span style={{ marginLeft: '8px' }}>{conflictCase.conflictType || '未知冲突'}</span>
                </div>
              </div>
            </Col>
            <Col span={8}>
        ...
        <Card title="详细说明" style={{ marginBottom: '20px' }}>
          <div style={{ marginBottom: '16px' }}>
            <Title level={5} style={{ marginBottom: '8px' }}>冲突类型说明</Title>
          ...
          <div style={{ marginBottom: '16px' }}>
            <Title level={5} style={{ marginBottom: '8px' }}>案件描述</Title>
            <Paragraph>
              {conflictCase.description || '无描述'}
            </Paragraph>
          </div>

          {conflictCase.conflictDetails && (
            <div>
              <Title level={5} style={{ marginBottom: '8px' }}>冲突详情</Title>
              <Paragraph>
                {conflictCase.conflictDetails || '无详情'}
              </Paragraph>
            </div>
          )}
        </Card>
        ...
        <Card title="风险评估" style={{ marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
            <ExclamationCircleOutlined style={{ marginRight: '8px', color: getRiskLevelColor(conflictCase.riskLevel || 'LOW') }} />
