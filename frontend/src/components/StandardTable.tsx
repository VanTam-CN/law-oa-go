...
export interface StandardTableProps
  extends Omit<TableProps<TableRecord>, 'columns' | 'dataSource'> {
  data: TableRecord[]
...
const StandardTable: React.FC<StandardTableProps> = ({
  ...
  const handleColumnVisibility = useCallback((key: string, visible: boolean) => {
    setVisibleColumns((prev) => prev.map((col) => (col.key === key ? { ...col, visible } : col)))
  }, [])
        ...
        if (col.render) {
          ...
          column.render = (value, record) => {
            ...
            if (
              ...
              col.dataIndex.includes('price') ||
              col.dataIndex.includes('money')
            ) {
        ...
        showHeader && (
          <div className='table-header'>
            <div className='header-left'>
              {title && (
                <Text strong className='table-title'>
                  {title}
                </Text>
              )}
            </div>
