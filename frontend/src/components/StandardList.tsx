    ...
    >
      <List.Item.Meta
        avatar={
          item.avatar ? (
            ...
            <Avatar
              style={{
                backgroundColor:
                  defaultStatusColors[item.status || 'default'] === 'success'
                    ? 'var(--color-success-100)'
                    : defaultStatusColors[item.status || 'default'] === 'error'
                      ? 'var(--color-error-100)'
                      : defaultStatusColors[item.status || 'default'] === 'warning'
                        ? 'var(--color-warning-100)'
                        : defaultStatusColors[item.status || 'default'] === 'processing'
        ...
        description={
          <div className='list-item-description'>
            {item.description && (
              <Paragraph ellipsis={{ rows: 2, expandable: false }} className='description-text'>
                {item.description}
              </Paragraph>
            )}
      ...
      {item.content && (
        <div className='list-item-content'>
          <Paragraph ellipsis={{ rows: 3, expandable: false }}>{item.content}</Paragraph>
        </div>
      )}
      {item.cover && <div className='list-item-cover'>{item.cover}</div>}
      {item.extra && <div className='list-item-extra'>{item.extra}</div>}
    </List.Item>
  )
