...
const TimeTracker: React.FC = () => {
  ...
  const submitEntry = (id: number) => {
    setTimeEntries((prev) =>
      prev.map((entry) => (entry.id === id ? { ...entry, status: 'SUBMITTED' } : entry)),
    )
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {record.status === 'DRAFT' && (
            <>
              ...
              <Button type='link' size='small' onClick={() => submitEntry(record.id)}>
                提交
              </Button>
              <Popconfirm title='确定删除吗？' onConfirm={() => deleteEntry(record.id)}>
      ...
      <Card>
        ...
        <Card title='快速计时' style={{ marginBottom: '24px' }}>
          <Form form={form} layout='inline'>
            ...
            <Form.Item>
              {isTracking ? (
                <Button type='primary' danger icon={<PauseCircleOutlined />} onClick={stopTimer}>
