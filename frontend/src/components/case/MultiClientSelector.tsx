...
const MultiClientSelector: React.FC<MultiClientSelectorProps> = ({
  ...
  // 通知外部变化
  const notifyChange = useCallback((newClients: SelectedClient[]) => {
  ...
  // 添加客户
  const addClient = useCallback((clientInfo: ClientInfo) => {
  ...
  // 更新客户角色
  const updateClientRole = useCallback((clientId: string, role: string) => {
    // 检查主要委托人限制
    ...
    const newClients = clients.map(c =>
      c.clientId === clientId ? { ...c, role } : c
    );
    notifyChange(newClients);
  }, [clients, allowPrimaryOnly, notifyChange]);

  // 更新客户信息
  const updateClientInfo = useCallback((clientId: string, updates: Partial<SelectedClient>) => {
    const newClients = clients.map(c =>
  ...
  // 打开冲突检测配置
  const openConflictConfig = useCallback((clientId: string) => {
      ...
      <Card
        title={
          <Space>
            <UserOutlined />
            <span>客户选择</span>
            <Tag color="blue">{clients.length}/{maxClients}</Tag>
          </Space>
        }
        extra={
          <Button
                ...
                <List.Item.Meta
                  title={
                    <Space>
                      ...
                      <Tag color={getRoleTagColor(client.role)}>
                        {getRoleLabel(client.role)}
                      </Tag>
                      <Tag color="default">{client.clientInfo.type === 'COMPANY' ? '企业' : '个人'}</Tag>
                    </Space>
                  }
                  description={
                    <div>
