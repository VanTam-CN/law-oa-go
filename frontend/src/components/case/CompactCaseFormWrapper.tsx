...
const CompactCaseFormWrapper: React.FC<CompactCaseFormWrapperProps> = ({
  ...
  const fallbackCaseTypes = [
    { value: '民事案件', label: '民事案件', causes: ['合同纠纷', '侵权责任', '婚姻家庭', '继承纠纷'] },
    { value: '商事案件', label: '商事案件', causes: ['公司纠纷', '金融纠纷', '投资纠纷', '证券纠纷'] },
    { value: '刑事案件', label: '刑事案件', causes: ['经济犯罪', '职务犯罪', '暴力犯罪', '网络犯罪'] },
    { value: '行政案件', label: '行政案件', causes: ['行政处罚', '行政许可', '信息公开', '征收补偿'] },
    { value: '知识产权', label: '知识产权案件', causes: ['商标侵权', '专利侵权', '著作权侵权', '商业秘密'] }
  ];
        ...
        >
          <div
            style={{
              ...
              background: currentStep === index ? '#1890ff' :
                       currentStep > index ? '#52c41a' : '#f0f0f0',
              color: currentStep === index ? 'white' :
                     currentStep > index ? 'white' : '#999',
              fontSize: '14px',
              fontWeight: '600',
              cursor: 'pointer',
          ...
          <Text
            style={{
              ...
              color: currentStep === index ? '#1890ff' :
                     currentStep > index ? '#52c41a' : '#666',
              textAlign: 'center'
            }}
          >
            {step.title}
                  ...
                  <Col span={12} key={category.value} style={{ marginBottom: 8 }}>
                    <Checkbox value={category.value}>
                      <Space direction="vertical" size={0}>
                        <Text strong style={{ fontSize: '13px' }}>{category.label}</Text>
          ...
          <Form.Item name="isHighRisk" valuePropName="checked">
            <Checkbox>
              <Space>
                ...
                <Text type="secondary" style={{ fontSize: '12px' }}>（需要合伙人审批）</Text>
              </Space>
            </Checkbox>
          </Form.Item>
        </Col>
