...
const SearchHistory: React.FC<SearchHistoryProps> = ({ onSelect, maxItems = 10 }) => {
  ...
  const formatFilters = (filters: any) => {
    ...
    if (filters.caseType) {
      filterTags.push(
        <Tag key='caseType' color='orange'>
          类型: {getCaseTypeLabel(filters.caseType)}
        </Tag>,
      )
    }
    if (filters.status) {
    ...
    if (filters.projectType) {
      filterTags.push(
        <Tag key='projectType' color='purple'>
          项目: {getProjectTypeLabel(filters.projectType)}
        </Tag>,
      )
    }
    if (filters.lawyerId) {
      filterTags.push(
        <Tag key='lawyer' color='cyan'>
          律师: {filters.lawyerId}
        </Tag>,
      )
    }
    ...
    if (filters.dateRange && filters.dateRange.length === 2) {
      filterTags.push(
        <Tag key='dateRange' color='magenta'>
          日期范围
        </Tag>,
      )
    }
    if (filters.amountRange && (filters.amountRange[0] || filters.amountRange[1])) {
      filterTags.push(
        <Tag key='amountRange' color='gold'>
          金额范围
        </Tag>,
      )
    }
      ...
      extra={
        <Space>
          ...
          {histories.length > 0 && (
            ...
            >
              <Tooltip title='清空历史'>
                <Button type='text' icon={<ClearOutlined />} size='small' danger />
              </Tooltip>
            </Popconfirm>
          )}
        </Space>
          ...
          renderItem={(item) => (
            ...
            >
              <List.Item.Meta
                ...
                description={
                  <div>
                    <div style={{ marginBottom: 4 }}>{formatFilters(item.filters)}</div>
...
export const withSearchHistory = (WrappedComponent: React.ComponentType<any>) => {
  return (props: any) => {
    ...
    return (
      <>
        <WrappedComponent {...props} addToSearchHistory={addToHistory} />
