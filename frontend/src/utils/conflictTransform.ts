...
export const detectClientType = (clientName: string, clientInfo?: any): ClientType => {
  ...
  const hasCompanyKeyword = companyKeywords.some(
    (keyword) =>
      clientName.includes(keyword) || clientName.toLowerCase().includes(keyword.toLowerCase()),
  )
...
export const parseOtherParties = (opponentInfo?: string): string[] => {
  ...
  // 先按换行符分割
  const lines = opponentInfo.split(/[\n\r]+/)
...
function isLikelyPartyName(text: string): boolean {
  ...
  const hasCompanySuffix = companySuffixes.some(
    (suffix) => text.includes(suffix) || text.toLowerCase().includes(suffix.toLowerCase()),
  )
