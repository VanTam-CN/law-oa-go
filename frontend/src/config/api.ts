...
// 根据环境选择配置
export const API_CONFIG: ApiConfig = ENV === 'production' ? PRODUCTION_CONFIG : DEVELOPMENT_CONFIG
...
export class ApiClient {
  ...
  static async post(endpoint: string, data: any): Promise<any> {
    try {
      ...
      if (!response.ok) {
        const errorMessage =
          HTTP_STATUS_MESSAGES[response.status as keyof typeof HTTP_STATUS_MESSAGES] ||
