...
import { Steps, Progress, Card, Typography, Space, Alert, Tooltip, Modal, message } from 'antd'
import {
  CheckCircleOutlined,
...
const ProgressIndicator: React.FC<ProgressIndicatorProps> = ({
  ...
  const calculateProgressStats = useCallback(
    (
      ...
      const { includeDisabled = true, useWeighting = false, weights = {}, minProgress = 0 } = config
  ...
  const stepsItems = useMemo(() => {
    return steps.map((step, index) => {
      ...
      return {
        ...
        onClick:
          enableStepNavigation && !step.disabled ? () => handleStepClick(step, index) : undefined,
      }
    })
  ...
  const handleStepClick = useCallback(
    async (step: StepConfig, index: number) => {
      if (!enableStepNavigation || step.disabled) {
        return
      }
  ...
  return (
    ...
    >
      {/* 步骤导航 */}
      <Card className='progress-steps-card' bordered={false} size={isCompact ? 'small' : 'default'}>
        <Steps
      ...
      {/* 进度条 */}
      {showProgressBar && (
        <Card className='progress-bar-card' bordered={false} size={isCompact ? 'small' : 'default'}>
          <Progress
            type={progressType}
            percent={stats.percentage}
            status={
              stats.errorSteps > 0 ? 'exception' : stats.percentage === 100 ? 'success' : 'active'
            }
                      ...
                      <FastForwardOutlined
                        ...
                        onClick={() => {
                          const nextIncompleteIndex = steps.findIndex(
                            (step, index) =>
                              index > currentStepIndex && !step.completed && !step.disabled,
                          )
