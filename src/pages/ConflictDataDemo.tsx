...
const mockConflictData = [
  ...
  },
  {
    ...
    RiskLevel: 'LOW',
    ConflictDetail:
      "律师 王芳 同时代理了案件 '中国建筑集团有限公司诉中国中铁股份有限公司项目竞标纠纷案'，存在潜在利益冲突",
    CreatedTime: '2025/11/5 10:22:31',
    Status: '进行中',
    LawyerName: '王芳',
  },
...
const ConflictDataDemo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const convertedConflicts = mockConflictData.map((legacyCase) => {
      ...
      return {
        ...
        riskLevel:
          legacyCase.RiskLevel === 'LOW'
            ? 'LOW'
            : legacyCase.RiskLevel === 'MEDIUM'
              ? 'MEDIUM'
              : 'HIGH',
        status: 'ACTIVE',
  ...
  const conflictColumns: ColumnsType<ConflictCase> = [
    {
      ...
      render: (_, record) => (
        <div>
          ...
          <div style={{ fontSize: '12px', color: '#666' }}>案件编号: {record.caseId}</div>
        </div>
      ),
    },
    {
      ...
      key: 'conflictType',
      render: (type) => <Tag color='red'>代理冲突</Tag>,
    },
    {
      title: '风险等级',
      dataIndex: 'riskLevel',
      key: 'riskLevel',
      render: (level) => <Tag color='green'>低风险</Tag>,
    },
    {
      title: '状态',
      dataIndex: 'status',
      key: 'status',
      render: (status) => <Badge status='processing' text='进行中' />,
    },
    {
      title: '创建时间',
      dataIndex: 'createdAt',
  ...
  const renderDataDemo = () => (
    <div>
      ...
      <Card
        ...
        extra={
          <Space>
            <Button icon={<WarningOutlined />}>模拟冲突检测</Button>
      ...
      >
        <div style={{ marginBottom: 16 }}>
          ...
          <Paragraph>
            <strong>风险原因：</strong>
            记录冲突情况；定期复查；保持警惕；建立预防措施；针对代理冲突类型案件制定专门处理流程
          </Paragraph>
          <Paragraph>
        ...
        <Table columns={conflictColumns} dataSource={conflicts} rowKey='id' pagination={false} />
      </Card>
    </div>
  )
  ...
  const renderIntegrationDemo = () => (
    <div>
      ...
      <Card title='功能说明'>
        <Paragraph>完整的集成功能演示将包括：</Paragraph>
        <ul>
          <li>冲突检测数据自动导入</li>
          <li>豁免申请一键生成</li>
          <li>审批流程完整追踪</li>
  ...
  "mitigationMeasures": "信息隔离：确保不同案件的信息严格隔离..."
        ...
        <ul>
          <li>
            <strong>字段映射：</strong>CaseID → id/caseId, CaseName → caseName
          </li>
          <li>
            <strong>类型转换：</strong>案件冲突 → REPRESENTATION_CONFLICT
          </li>
          <li>
            <strong>状态映射：</strong>进行中 → ACTIVE
          </li>
          <li>
            <strong>数据结构：</strong>扁平结构 → 嵌套结构（parties数组）
          </li>
          <li>
            <strong>默认值：</strong>自动生成缺失的字段（mitigationMeasures等）
          </li>
        </ul>
        ...
        <ul>
          <li>
            <strong>ConflictDetectionAdapter：</strong>处理API调用和数据转换
          </li>
          <li>
            <strong>ConflictWaiverAdapter：</strong>后端数据适配器
          </li>
          <li>
            <strong>ConflictWaiverService：</strong>业务逻辑集成服务
          </li>
          <li>
            <strong>EnhancedWaiverApprovalInterface：</strong>增强型UI组件
          </li>
        </ul>
      </Card>
    </div>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Paragraph>基于你提供的实际冲突检测数据，演示如何与豁免审批系统进行集成。</Paragraph>
          ...
          <div>
            <Descriptions column={2} bordered>
              ...
              <Descriptions.Item label='相关方' span={2}>
                <Tag style={{ margin: '4px 4px 4px 0' }}>王芳 (律师)</Tag>
              </Descriptions.Item>
            ...
            <Alert
              ...
              description={
                <div>
                  <p>
                    <strong>豁免类型：</strong>REPRESENTATION_CONFLICT（代理冲突）
                  </p>
                  <p>
                    <strong>风险等级：</strong>LOW（低风险）
                  </p>
                  <p>
                    <strong>建议措施：</strong>
                    信息隔离，确保不同案件的信息严格隔离；建立定期沟通机制
                  </p>
                </div>
              }
              type='info'
