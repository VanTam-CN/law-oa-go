...
const FileManagement: React.FC = () => {
  ...
  // 最简单的函数定义
  const fetchFiles = async (pageNum: number = 1, pageSize: number = 10, category?: string) => {
    try {
      setLoading(true)
  ...
  const handleUpload = async (
    ...
    try {
      setUploading(true)
      const response: UploadResponse = await uploadFile(file, category, description, customName)
