Create Codex pet row `running-right` for `siam`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: preserve the existing pixel-art Siamese kitten: cream fur, dark brown mask, ears, paws and curled tail, bright blue eyes, chunky dark outline, magenta chroma background; expand expressions and actions while preserving identity. Preserve silhouette, face, palette, material, proportions, markings, and props.
