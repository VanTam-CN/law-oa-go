...
`cmd/qa-fixture` 是非生产环境的可重复验收数据入口，用于验证律师 A、律师 B 和独立冲突核查人 C 的真实操作边界。它不是数据库迁移，也不会由应用启动或生产 schema bootstrap 自动执行。
...
6. 以 C 完成一次“信息不足”复核，确认审批仍被阻断，后端不能通过直接 API 绕过前端按钮。
7. 保存浏览器、HTTP 状态码、审计日志和数据库核验结果，形成验收报告。

## 重复执行规则

夹具使用固定的 `qa.invalid` 账号、案件编号和检测编号，重复执行会更新这些虚构对象并保留审计/索引证据，不会清空数据库，也不会删除正式业务数据。若 QA 数据库中混入真实业务数据，应先换用新的隔离数据库，不要用夹具命令清理。

运行 `-mode verify` 只读核验三角色、隔离案件、冲突检测、审批门禁和四类冲突范围；它不会修复数据。
